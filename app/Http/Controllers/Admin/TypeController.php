...
class TypeController extends Controller{
    ...
    public function __construct(){
        ...
        $this->pageLayout = 'admin.pages.type.';
        $this->middleware('auth');
    }

    /* ----------------------------------------------------------------------------------
    @Description: Function Index InWards Page
    ---------------------------------------------------------------------------------- */
    public function index_inward(Builder $builder, Request $request){
        ...
        $transaction = Transaction::where('type','inward')->orderBy('id','desc');
        if (request()->ajax()) {
        ...
        ->parameters(['order' =>[],]);
        return view($this->pageLayout.'indexInward',compact('html','transaction_balance'));
    }

    /* ----------------------------------------------------------------------------------
    @Description: Function Index OutWards Page
    ---------------------------------------------------------------------------------- */
    public function index_outward(Builder $builder, Request $request){
        $select_raw = "cu.`currency`, SUM(COALESCE(CASE WHEN transaction.`type` = 'outward' THEN transaction.amount END,0)) total_outword , SUM(COALESCE(CASE WHEN transaction.`type`= 'inward' THEN transaction.amount END,0)) total_inword , SUM(COALESCE(CASE WHEN transaction.`type`= 'inward' THEN transaction.amount END,0)) - SUM(COALESCE(CASE WHEN transaction.`type`= 'outward' THEN transaction.amount END,0)) as balance";
        $transaction_balance = Transaction::selectRaw($select_raw)->from('transaction', 'transaction')->leftJoin('currency as cu','cu.id','=','transaction.currency_id')->groupBy('transaction.currency_id')->get();
        $transaction = Transaction::where('type','outward')->orderBy('id','desc');
        if (request()->ajax()) {
            return DataTables::of($transaction->get())
            ->addIndexColumn()
            ->editColumn('date', function (Transaction $transaction) {
        ...
        ->parameters(['order' =>[],]);
        return view($this->pageLayout.'indexoutward',compact('html','transaction_balance'));
    }

    /* ----------------------------------------------------------------------------------
    @Description: Function Index Other Page
    ---------------------------------------------------------------------------------- */
    public function index_other(Builder $builder, Request $request){
        $transaction = Transaction::where('type','other')->orderBy('id','desc');
        if (request()->ajax()) {
            return DataTables::of($transaction->get())
            ->addIndexColumn()
            ->editColumn('date', function (Transaction $transaction) {
        ...
        ->parameters([
            ...
            'dom'  => 'Bfrtip',
            'buttons'     => [
                ['extend' => 'print','title' => ' | USERS LISTING','exportOptions' => ['columns'=> [1,2,3,4,5,6]]],
                ['extend' => 'csv','title' => '| USERS LISTING','exportOptions' => ['columns'=> [1,2,3,4,5,6]]],
                ['extend' => 'excel','title' => ' | USERS LISTING','exportOptions' => ['columns'=> [1,2,3,4,5,6]]],
                ['extend' => 'pdf','title' => ' | USERS LISTING','exportOptions' => ['columns'=> [1,2,3,4,5,6]] ]
            ],
            'searching'   => true,
            'data' => false
        ]);
        return view($this->pageLayout.'indexother',compact('html'));
    }
}